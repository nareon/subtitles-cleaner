...
# Keep batches small to avoid exceeding context or HTTP body limits on the LLM
# server. A larger batch combined with the verbose prompt can easily go over
# the model's max tokens or the API's max request size.
BATCH_SIZE = 8
MAX_WORKERS = 16
MAX_RETRIES = 3
REQUEST_TIMEOUT = 90
...
PROMPT = """
Clasificar la frase:
- false — mala (nombre propio, apelativo, número en cifras, abreviatura, palabra extranjera, jerga, símbolo especial o marca)
...
Devuelve solo JSON con la estructura:
{
  "phrase": "string",
  "isGood": "boolean",
  "reason": "string"
